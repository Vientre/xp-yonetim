...
export default function AylikTabloPage() {
  ...
  // Fetch entries for selected month + business
  ...
  // Build full day grid (all days of month)
  const daysCount = getDaysInMonth(year, month)
  ...
  return (
    <div className="p-6 space-y-5 print:p-2 print:space-y-3">
      ...
      <div className="hidden print:block text-center mb-4">
        <h1 className="text-xl font-bold">{BUSINESSES.find(b => b.id === businessId)?.name}</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 print:grid-cols-6 print:gap-2">
        {[
          ...
          <div key={c.label} className={cn("rounded-xl p-3 print:p-2", c.bg)}>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="text-left px-4 py-3 font-semibold text-slate-600 w-24">Tarih</th>
                  <th className="text-left px-3 py-3 font-semibold text-slate-600 w-12 print:hidden">Gün</th>
                  ...
                  <th className="px-3 py-3 print:hidden w-8"></th>
                </tr>
              </thead>
              <tbody>
                {Array.from({ length: daysCount }, (_, i) => {
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        {entry ? (
                          <>
                            ...
                            <td className="px-4 py-2.5 text-right text-slate-700">
                              {entry.ticketIncome > 0 ? `₺${formatTL(entry.ticketIncome)}` : <span className="text-slate-300">—</span>}
                            </td>
                      ...
                      {entry && isExpanded && (
                        <tr key={`${dateStr}-detail`} className="bg-blue-50/60 border-b border-blue-100">
